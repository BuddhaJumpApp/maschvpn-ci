...
三件套：`fjolskylduoryggisverndar/maschvpn`（源码，私有）→ **`BuddhaJumpApp/maschvpn-ci`（本仓库，公开）** → `fjolskylduoryggisverndar/maschvpn-web`（官网快照，域名 `maschvpn.xyz`）。
...
- 发送方必须向 `api.github.com/repos/BuddhaJumpApp/maschvpn-ci/dispatches` POST `event_type: pubspec-updated`。
...
gh workflow run build.yml --repo BuddhaJumpApp/maschvpn-ci --ref master
...
# 看进度
gh run list --repo BuddhaJumpApp/maschvpn-ci --workflow build.yml --limit 5
gh run watch --repo BuddhaJumpApp/maschvpn-ci <run-id>
```
...
官网 → `dl.maschvpn.xyz/android|windows` → Cloudflare Worker `fjolsky-downloads` → 302 → `edgeone.gh-proxy.org/https://github.com/BuddhaJumpApp/maschvpn-ci/releases/download/<tag>/<asset>`。
